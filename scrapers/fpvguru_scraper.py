"""
Scraper for fpvguru.in — WooCommerce / Woodmart theme
Paginates /shop/page/N/, extracts product cards, then fetches each product page for full specs.
Output: scrapers/output/fpvguru.json

Resume behaviour:
  - Saves a checkpoint file (fpvguru_checkpoint.json) after every scraped product.
  - On restart, skips already-scraped URLs and merges with previous results.
  - If the site is unreachable (blocked/down), waits RETRY_WAIT_HOURS and tries again
    automatically — indefinitely, until the full run completes.
"""
import json
import re
import time
...
BASE_URL          = "https://fpvguru.in"
STORE_ID          = "fpvguru"
OUTPUT_DIR        = os.path.join(os.path.dirname(__file__), "output")
OUTPUT_FILE       = os.path.join(OUTPUT_DIR, "fpvguru.json")
CHECKPOINT_FILE   = os.path.join(OUTPUT_DIR, "fpvguru_checkpoint.json")
RETRY_WAIT_HOURS  = 1          # hours to wait when site is unreachable
CONNECT_TIMEOUT   = 15         # seconds per request
CONSEC_FAIL_LIMIT = 5          # consecutive failures before declaring site blocked
...
def load_checkpoint() -> tuple[list[str], list[dict]]:
    """Return (scraped_urls, products) from checkpoint, or empty lists."""
...
def site_is_reachable(session: requests.Session) -> bool:
    """Quick check: can we reach the shop homepage?"""
    try:
        resp = session.get(
            f"{BASE_URL}/shop/",
...
def wait_for_site(session: requests.Session) -> None:
    """Block until the site is reachable, sleeping RETRY_WAIT_HOURS between checks."""
    attempt = 1
    while not site_is_reachable(session):
        ...
        attempt += 1
    print(f"  [unblocked] Site is reachable again — resuming scrape.")
...
def collect_product_urls(session: requests.Session) -> list[str]:
    ...
    while True:
        ...
        if soup is None:
            # Could be blocked — wait and retry this page
...
def main():
    ...
    print("=== FPV Guru Scraper (with resume + auto-retry) ===")

    # Wait until site is reachable before even starting
    ...
    if not product_urls:
        print("No products found — site may still be blocking.", file=sys.stderr)
        sys.exit(1)

    # Step 2: scrape only URLs not already in checkpoint
    ...
    print(f"\n✓ Scraped {len(products)} products from FPV Guru")
